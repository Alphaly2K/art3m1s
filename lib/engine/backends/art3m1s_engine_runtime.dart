...
void _dispatchMediaCommand(String kind, String rawPayload) {
  ...
  final payload = decoded is Map
      ...
      : <String, dynamic>{};
  Art3m1sEngineRuntime._activeRuntime?.media.handleCommand(kind, payload);
}
...
void _dispatchUiCommand(String kind, String rawPayload) {
  ...
  final payload = decoded is Map
      ? decoded.map((key, value) => MapEntry(key.toString(), value))
      : <String, dynamic>{};
  final bridge = Art3m1sEngineRuntime._activeRuntime;
...
Future<void> _openInBrowser(String url) async {
  try {
    if (Platform.isMacOS) {
      ...
      Log.warn('[Art3m1sEngineRuntime] openbrowser 在此平台不支持: $url');
    }
  } catch (e) {
    Log.error('[Art3m1sEngineRuntime] openbrowser 失败: $e');
  }
}
...
/// Artemis core backend for the unified host engine contract.
///
/// Owns the dynamic library, versioned FFI table, runtime/resource handles,
/// host event queue, shared texture bridge, input gate, and media adapter.
class Art3m1sEngineRuntime implements EngineRuntime {
  ...
  static Art3m1sEngineRuntime? _activeRuntime;

  Art3m1sEngineRuntime({
    this.onDialogRequested,
  ...
  static Future<List<String>> listArchiveEntries(String archivePath) async {
    final bridge = PfsBridge();
    final archive = bridge.open(archivePath);
    if (archive.address == 0) return const <String>[];
    try {
      final count = bridge.entryCount(archive);
      if (count <= 0) return const <String>[];
      return <String>[
        for (var index = 0; index < count; index++)
          ?bridge.entryPath(archive, index),
      ];
    } finally {
  ...
  static Future<Uint8List?> readArchiveFile(
    String archivePath,
    String relativePath,
  ) async {
  ...
  Future<int?> enableSharedTexture({
    ...
    if (coreApi != null) {
      ...
      try {
        ...
        Log.info('[Art3m1sEngineRuntime] 当前 core 不支持共享纹理，使用 RGBA 回读: $error');
        return null;
      }
    }
    ...
    switch (call.method) {
      ...
      case 'surfaceAvailable':
        ...
        if (descriptor is Map && !_attachSharedTexture(descriptor)) {
          Log.warn('[Art3m1sEngineRuntime] 无法重新绑定 Android 共享纹理 surface');
        }
    }
  }
    ...
    if (result > 0 && (_sharedTextureKind == 2 || _sharedTextureKind == 3)) {
      unawaited(
        _sharedTextureChannel.invokeMethod<void>('frameAvailable').catchError((
          ...
          Log.warn('[Art3m1sEngineRuntime] 共享纹理帧通知失败: $error');
        }),
      );
    } else if (result < 0) {
      Log.warn('[Art3m1sEngineRuntime] 共享纹理提交失败，回退到 RGBA 路径');
      _detachSharedTexture();
    }
    return result;
  }
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      Log.info('[Art3m1sEngineRuntime] 使用版本化 C ABI v1');
      ...
      Log.error('[Art3m1sEngineRuntime] Core 库加载失败: $e');
      _initialized = false;
      return;
    }
    try {
      ...
      Log.error('[Art3m1sEngineRuntime] host events v1 初始化失败: $error');
      _initialized = false;
    }
  }
  ...
  bool _tryEnableHostEvents() {
    ...
    try {
      ...
      if (handle == nullptr) {
        Log.error('[Art3m1sEngineRuntime] 创建 host events 句柄失败');
        return false;
      }
      ...
      _syncHostState();
      _hostEvents = api;
      Log.info('[Art3m1sEngineRuntime] 使用无反向回调 host events 通道');
      return true;
    } catch (error) {
      if (handle != nullptr) {
        try {
      ...
      Log.error('[Art3m1sEngineRuntime] host events v1 启用失败: $error');
      return false;
    }
  }
  ...
  void _syncTextTranslation() {
    ...
    try {
      ...
      if (api.setTextReplacements(ptr, bytes.length) == 0) {
        Log.warn('[Art3m1sEngineRuntime] 翻译替换表提交失败');
      }
  ...
  void _drainHostEvents() {
    ...
    try {
      ...
      Log.error('[Art3m1sEngineRuntime] host event 拉取失败: $error');
    } finally {
      malloc.free(output);
      calloc.free(count);
    }
  ...
  bool setFontOverride(Uint8List bytes) {
    ...
    try {
      ...
      Log.info('[Art3m1sEngineRuntime] 当前 core 不支持运行时字体覆盖: $error');
      return false;
    }
  }
  ...
  void setReportedOs(String? os) {
    ...
    try {
      ...
      Log.warn('[Art3m1sEngineRuntime] 当前 core 不支持机种上报覆盖: $error');
    }
  }
  ...
  bool setProfilerEnabled(bool enabled) {
    ...
    try {
      ...
      Log.info('[Art3m1sEngineRuntime] 当前 core 不支持 Profiler: $error');
      return false;
    }
  }
  ...
  ProfilerSnapshot? readProfilerSnapshot() {
    ...
    try {
      ...
      _profilerSymbolsUnavailable = true;
      Log.warn('[Art3m1sEngineRuntime] Profiler 快照读取失败: $error');
      return null;
    }
  }
  ...
  void setSaveDir(String dir) {
    if (_lib == null) {
      Log.warn('[Art3m1sEngineRuntime] setSaveDir: _lib is null');
      return;
    }
    try {
      // 确保目录存在
      ...
      Log.info('[Art3m1sEngineRuntime] 存档目录已设置: $dir');
    } catch (e) {
      Log.error('[Art3m1sEngineRuntime] setSaveDir 失败 ($dir): $e');
    }
  }
  ...
  void createRuntime(int stageW, int stageH, {int backend = 0}) {
    ...
    if (coreApi?.hasRuntimeMedia ?? false) {
      ...
      Log.info('[Art3m1sEngineRuntime] runtime 视频解码已启用');
    } else {
      try {
        ...
        Log.info('[Art3m1sEngineRuntime] 当前 core 不支持 runtime 视频解码，保留宿主路径');
      }
    }
  }
  ...
  bool setRenderQuality(EngineRenderQuality quality) {
    ...
    try {
      final coreApi = _coreApi;
      if (coreApi != null) {
        return coreApi.setRenderQualityPreset(_runtime!, quality.index) != 0;
      }
      final fn = _lib!
          .lookupFunction<
      ...
      return fn(_runtime!, quality.index) != 0;
    } catch (error) {
      Log.warn('[Art3m1sEngineRuntime] 渲染质量设置不可用: $error');
      return false;
    }
  }
  ...
  int backendCapabilities() {
    ...
    try {
      ...
      Log.warn('[Art3m1sEngineRuntime] GPU capabilities 查询不可用: $error');
      return 0;
    }
  }
  ...
  bool configureSpatialUpscale(double renderScale, {double sharpness = 0}) {
    ...
    try {
      ...
      Log.warn('[Art3m1sEngineRuntime] spatial upscale 配置不可用: $error');
      return false;
    }
  }
  ...
  /// 独立于 player 的 Art3m1sEngineRuntime：只用 lib + 进程级 FileProvider，不动 _activeRuntime。
  Future<String?> probeCaption({
    ...
    try {
      ...
      Log.warn('[Art3m1sEngineRuntime] probeCaption 读取 system.ini 失败: $e');
      FileProvider.close();
      return null;
    }
    ...
    try {
      ...
      Log.warn('[Art3m1sEngineRuntime] probeCaption 扫描语言表失败，将回退 headless: $e');
    }
    if (tableCaption != null) {
      Log.info('[Art3m1sEngineRuntime] 从语言表获取 caption: $tableCaption');
      FileProvider.close();
      return tableCaption;
    }
    ...
    try {
      ...
      Log.warn('[Art3m1sEngineRuntime] probeCaption 加载库失败: $e');
      FileProvider.close();
      return null;
    }
    final lib = _lib;
    ...
    try {
      ...
      Log.warn('[Art3m1sEngineRuntime] probeCaption 调用失败: $e');
      return null;
    } finally {
      malloc.free(iniPtr);
      malloc.free(platPtr);
  ...
  void _updateStageSize() {
    ...
    try {
      ...
      Log.info('[Art3m1sEngineRuntime] 舞台尺寸已更新: $_stageWidth x $_stageHeight');
    } catch (e) {
      Log.warn('[Art3m1sEngineRuntime] 查询舞台尺寸失败: $e');
    }
  }
  ...
  void notifyLifecycle(int state) {
    ...
    try {
      ...
      Log.warn('[Art3m1sEngineRuntime] notifyLifecycle 不可用: $e');
    }
  }
  ...
  void shutdown() {
    ...
    if (runtime != null && lib != null) {
      try {
        Log.info('[Art3m1sEngineRuntime] runtime destroy begin');
        if (coreApi != null) {
        ...
        Log.info('[Art3m1sEngineRuntime] runtime destroy end');
      } catch (e) {
        Log.warn('[Art3m1sEngineRuntime] runtime destroy failed: $e');
    ...
    if (hostEventsHandle != null && coreApi != null) {
      try {
        ...
        Log.warn('[Art3m1sEngineRuntime] host event destroy failed: $e');
      }
    }
    FileProvider.close();
    if (resources != null && resources != nullptr) {
      try {
        ...
        Log.warn('[Art3m1sEngineRuntime] resource destroy failed: $e');
      }
    }
    _lib = null;
  }

...
- 对 Android 目标新增 Miuix Theme。
- 新增游戏 Manifest 文件支持，可以一键导入游戏配置。
- 新增了游戏输入门控支持，屏蔽特定输入，防止一些移动端移植的游戏由于引擎默认键盘行为致使游戏脚本执行异常。
- 移动端新增双指拖动手势。(貌似有问题，将很快修复)

### 修复

- 修复了 FPS 指示器在特定屏幕上可见性不佳的问题。
- 修复了一些 UI 错误逻辑。

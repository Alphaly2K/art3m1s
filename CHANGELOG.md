...
- 游戏导入全平台统一为「选择文件夹 → 探测 → 原地入库」，不再复制游戏文件；
  目录导入同时识别解包工程（`system.ini`/`.hcb`）和打包成 PFS 归档的 Artemis
  游戏，不再提供单独选择 `.pfs` 文件的入口。Android 改用「所有文件访问」授权 +
  SAF 目录选择 +
  真实路径解析；SAF 整树复制、未完成标记清理、拷贝进度和入库复制全部退役。
  早期 Android 沙箱内的遗留副本仍可游玩，删除库条目时只清理这些托管副本，
  绝不动用户原目录。
...
- macOS、Windows 和 Linux 窗口化时保留平台原生壳，进入全屏后自动切换为
  PS5 风格桌面大屏界面；PS5 壳支持鼠标、键盘和手柄焦点导航。
- 桌面端新增 PS5 风格自绘文件浏览器；进入大屏后游戏导入、封面、翻译对照和
  覆盖字体不再调用系统原生 FilePicker。
...
- Android 从资料库移除项目时删除整个 SAF 导入目录，不触碰用户原目录。
- iOS 按路径去重导入，避免同一游戏重复进入资料库。
- Android 未完成的 SAF 导入会留下标记，下次启动时清理未成功的拷贝。
- iOS 资料库改为直接扫描 App 文件夹，并移除其它导入入口。
- Miuix 资料库加号与大标题对齐。
- 关于页第三方许可证补充 art3m1s-core 的 Rust 依赖。
- 不支持空间超分的平台不再显示超分设置。
- Miuix 列表项去掉分割线；iOS 分组列表保留分割线，并改为左右等距。
